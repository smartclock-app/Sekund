...
- The screen can be pinned to just Sekund plus Settings and whatever browser(s) are installed, via Android's Lock Task API — no way to reach any other app. Lock Task's default feature set (`LOCK_TASK_FEATURE_NONE`) additionally blocks the notification shade / Quick Settings and the home button entirely; `setLockTaskFeatures()` re-enables `SYSTEM_INFO`, `NOTIFICATIONS`, `HOME`, and `OVERVIEW` so Quick Settings (and USB/wireless debugging within it) stays reachable — `NOTIFICATIONS` can only be enabled together with `HOME`, which is harmless here since Sekund is also the persistent default Home app, so pressing Home just re-focuses it.
...
The status bar itself is deliberately **not** disabled via `setStatusBarDisabled()`, even though Device Owner supports it and Android documents it as the standard kiosk-device setting — that API would take away Quick Settings entirely, which is the opposite of what the `setLockTaskFeatures()` call above restores.
...
Quick Settings and the full Settings app (one of the whitelisted packages) both stay reachable, so Developer Options and USB/wireless debugging specifically can be reached either way. There's no in-app toggle for debugging: `adb`-related settings persist across reboots (they're stored in the settings database, not reset at boot), and debugging has to already be enabled to run the `dpm set-device-owner` command above in the first place, so there's nothing left to wire up.
